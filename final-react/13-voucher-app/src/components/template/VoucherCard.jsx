...
export default function VoucherCard() {
  ...
  return (
    <>
    {!isLoading &&     <Card className="w-full bg-background shadow-none p-0 border-none mt-5">
      <CardHeader className="flex flex-row justify-between items-start p-0">
        <div>
          <h1 className="text-xl font-bold tracking-tight text-gray-900 dark:text-white">Invoice ID</h1>
          <p className="text-gray-700 dark:text-gray-300">{data.voucherId}</p>
        </div>
        <div className="text-right">
          <p className="font-medium text-gray-700 dark:text-gray-300">Invoice to</p>
          <p className="font-medium text-gray-900 dark:text-white">{data.customerName}</p>
          <p className="text-gray-700 dark:text-gray-300">Date: {data.saleDate}</p>
        </div>
      </CardHeader>
      ...
      <CardContent className="space-y-6 p-0">
        ...
        <TableHeader>
          ...
          <TableBody>
        {
            data.records.map((item) => <Voucher_Card_Row key={item.id} voucher={item}/>)
        }
          </TableBody>
        </Table>
        ...
        <div className="space-y-4 pt-4">
          <h3 className="font-medium text-gray-900 dark:text-white">Payment Transfer to</h3>
      ...
      <CardFooter className="flex justify-center pt-6 pb-4">
        <p className="text-blue-600 dark:text-blue-400 font-medium">Thanks to You</p>
      </CardFooter>
    </Card>}
    </>
  )
